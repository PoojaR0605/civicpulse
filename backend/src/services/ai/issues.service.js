...
// Stub — Phase 3 replaces with real Firebase upload
const uploadPhoto = async (file) => {
  if (!file) return { url: null, path: null };
  // TODO Phase 3: upload to Firebase Storage
  return {
...
// SLA deadline based on category config
...
const IssuesService = {
  async submit({ reportedBy, latitude, longitude, gpsAccuracy, category,
                 title, description, address, file }) {
    // 1. Route to ward
    const ward = await WardModel.findByCoordinates(latitude, longitude);

    // 2. Upload photo (stub for now)
